(function(){
  ...
  function seedContents(){
    return {
      hotelName:'Grand Meridian Resort',
  ...
  function seedSettings(){ return {currency:'USD',taxRate:0.1,siteName:'Grand Meridian Resort',supportEmail:'support@grandmeridian.com',maintenanceMode:false,allowRegistration:true,gridColumns:3}; }
  function seedOrders(){
  ...
  function getRooms(){ return LS.get('hv_rooms', LS.get('hv_seedbase',{rooms:[]}).rooms); }
  function saveRooms(r){ LS.set('hv_rooms',r); }
  function getUsers(){ return LS.get('hv_users', LS.get('hv_seedbase',{users:[]}).users); }
  function saveUsers(u){ LS.set('hv_users',u); }
  function getBookings(){ return LS.get('hv_bookings',[]); }
  function saveBookings(b){ LS.set('hv_bookings',b); }
  function getPromotions(){ return LS.get('hv_promos', LS.get('hv_seed',{promotions:[]}).promotions); }
  function savePromotions(p){ LS.set('hv_promos',p); }
  function getPromoCodes(){ return LS.get('hv_promocodes', LS.get('hv_seed',{promoCodes:[]}).promoCodes); }
  function savePromoCodes(p){ LS.set('hv_promocodes',p); }
  function getGallery(){ return LS.get('hv_gallery', LS.get('hv_seed',{gallery:[]}).gallery); }
  function saveGallery(g){ LS.set('hv_gallery',g); }
  function getFood(){ return LS.get('hv_food', LS.get('hv_seed',{food:[]}).food); }
  ...
  function getContents(){ return LS.get('hv_contents',seedContents()); }
  function saveContents(c){ LS.set('hv_contents',c); }
  function getWifi(){ return LS.get('hv_wifi',seedWifi()); }
  ...
  function getOrders(){ return LS.get('hv_orders',[]); }
  ...
  function roomById(id){ return getRooms().filter(function(r){return r.id===id;})[0]; }
  ...
  function cookieUser(){
    try{
      ...
      for(var i=0;i<m.length;i++){
        var p=m[i].indexOf('=');
        if(p===-1)continue;
        var k=m[i].substring(0,p), v=decodeURIComponent(m[i].substring(p+1));
  ...
  function getCurrentUser(){
    var cu=cookieUser();
    if(cu&&cu.email){
      var locals=getUsers().filter(function(x){return x.email&&x.email.toLowerCase()===cu.email.toLowerCase();});
      if(locals.length&&locals[0].id){
        var out={}; for(var k in cu)out[k]=cu[k]; out.id=locals[0].id;
        return out;
      }
      return cu;
    }
  ...
  function logout(){ LS.remove(CURRENT_USER_KEY); }
  ...
  function liveStay(b){
    ...
    if(hasOut){
      ...
      else if(todayISO()===b.checkOut){ state='CHECK_OUT_TODAY'; target=coutDT-now; }
      else { state='CURRENT_STAY'; target=coutDT-now; }
    } else {
      if(now<cinDT){
        if(todayISO()===b.checkIn){ state='CHECK_IN_TODAY'; target=cinDT-now; }
        else { state='UPCOMING'; target=cinDT-now; }
      } else {
    ...
    return {state:state,remaining:countdown(target),progress:prog,target:target,meta:STAY_STATES[state]||STAY_STATES.UPCOMING,checkIn:cinDT,checkOut:coutDT};
  }
  ...
  function openConfirm(title,msg,options){
    ...
    var m=openModal(
      '<div class="modal sm"><div class="modal-head"><h3>'+esc(title)+'</h3><button class="modal-x" aria-label="Close">&times;</button></div>'+
      '<div class="modal-body"><p style="font-size:.92rem;color:var(--muted)">'+msg+'</p></div>'+
      '<div class="modal-foot"><button class="btn btn-ghost" data-c="1">Cancel</button><button class="btn '+okBtn+'" data-ok="1">'+esc(btnTxt)+'</button></div></div>'
    );
    return new Promise(function(res){
  ...
  function redirectFor(role){
    ...
    if(role==='STAFF') return '/admin/booking-management';
  ...
  var USER_NAV=[
    ...
    {href:'/user/profile',icon:'settings',label:'Settings',key:'uset'}
  ];
  var STAFF_NAV=[
    {href:'/admin/booking-management',icon:'home',label:'Dashboard',key:'sd'},
    {label:'OPERATIONS'},
    ...
    {label:'ACCOUNT'},
    {href:'/user/profile',icon:'user',label:'Profile',key:'apro'},
    {href:'/user/profile',icon:'settings',label:'Settings',key:'aset'}
  ];
  var ADMIN_NAV=[
  ...
  function topbarHTML(title,crumb){
    ...
    var crumbHtml=crumb?crumb.map(function(x,i){
      if(i===crumb.length-1)return '<span class="cur">'+esc(x)+'</span>';
    ...
    var drop=''+
      ...
      '<a href="/user/profile">'+icon('user','icon')+' My Profile</a>'+
      '<a href="'+(cur.role==='GUEST'||cur.role==='STAFF'?'/user/profile':'/admin/settings')+'">'+icon('settings','icon')+' Settings</a>'+
    ...
    return '<button class="icon-btn burger" data-burger title="Menu">'+icon('menu','icon')+'</button>'+
      ...
      '<button class="icon-btn" onclick="location.href=\'/user/notifications\'" title="Notifications">'+icon('bell','icon')+(unread?'<span class="dot"></span>':'')+'</button>'+
      drop+'</div>';
  }
  function renderDashChrome(active,title,crumb){
    ...
    if(sb){
      sb.innerHTML='<div class="sb-logo"><span class="mark logo" style="width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,var(--primary),#0a3a37);color:#fff;display:inline-flex;align-items:center;justify-content:center">'+icon('feather','icon')+'</span><div><div style="font-weight:800;font-size:.95rem;line-height:1.1">'+esc(getFilesContents().hotelName)+'</div><div class="text-xs" style="color:var(--muted);letter-spacing:.14em;text-transform:uppercase;font-weight:700">'+esc(roleLabel(role))+'</div></div></div>'+
        ...
        '<div class="sb-footer"><a href="/user/profile" class="sb-user">'+avatarHTML(u,'avatar sm')+'<div><div class="uname">'+esc(u.name)+'</div><div class="urole">'+esc(roleLabel(role))+'</div></div></a>'+
  ...
  function wifiForBooking(b){
    ...
    if(!b) return w.hotel;
    var room=roomById(b.roomId);
    if(w.rooms[b.roomId]) { var rw=w.rooms[b.roomId]; return {ssid:rw.ssid,password:rw.password,instructions:w.hotel.instructions,label:'Room network \u2022 '+room.name}; }
    if(room&&w.floors[String(room.floor)]) { var fw=w.floors[String(room.floor)]; return {ssid:fw.ssid,password:fw.password,instructions:w.hotel.instructions,label:'Floor '+room.floor+' network'}; }
    return {ssid:w.hotel.ssid,password:w.hotel.password,instructions:w.hotel.instructions,label:'Hotel-wide network'};
  }
  ...
  window.App = {
    ...
    getGallery:getGallery, saveGallery:saveGallery, getFood:getFood, saveFood:saveFood, getNotifs:getNotifs, saveNotifs:saveNotifs,
